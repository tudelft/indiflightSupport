...
from natnet3x.NatNetClient import NatNetClient
...
parser.add_argument('-udp', '--udp', dest='udp_msg', action='store_true', help="send FAKE_GPS over udp, and nothing over ivy.")
...
parser.add_argument('-t', '--test', dest='test', action='store_true', help="Test udp mode")
...
if args.udp_msg:
    ...
    # gonna need ivy and all other dependencies
...
def receiveRigidBodyList( rigidBodyList, stamp ):
    global j, last_pos, last_time_ms
    for (ac_id, pos, quat, valid) in rigidBodyList:
        ...
        if args.rgl_msg:
            ...
            CYBERZOO_LAT = 51.99071002805145
            CYBERZOO_LON = 4.376727452462819
            RE = 6378137.
            msg = FAKE_GPS.copy()
            msg['time_ms'] = int(1000. * stamp)
            # assume earth as a sphere
            msg['lat'] = int( 1e7 * (
                CYBERZOO_LAT + 180/np.pi * (pos[1] / RE)
            ))
            msg['lon'] = int( 1e7 * (
                CYBERZOO_LON \
                + 180/np.pi * (pos[0] / RE) / np.cos(np.pi/180 * CYBERZOO_LON)
            ))
            msg['altCm'] = int(100. * pos[2])
            msg['hdop'] = 5
            msg['groundSpeed'] = int(100 * np.hypot(vel[0], vel[1]))
            # convert quaternion to psi euler angle
            dcm_0_0 = 1.0 - 2.0 * (quat[1] * quat[1] + quat[2] * quat[2])
            dcm_1_0 = 2.0 * (quat[0] * quat[1] - quat[3] * quat[2])
            msg['groundCourse'] = int(10 \
            ...
            delta_pos = [pos[0]-last_pos[0], pos[1]-last_pos[1], pos[2]-last_pos[2]]
            delta_pos_norm = np.linalg.norm(delta_pos)
            delta_t = (msgE['time_ms'] - last_time_ms)/1000
            v_norm = delta_pos_norm/delta_t
            print(f'Vel: {v_norm:.2f}')
            last_pos = pos
            last_time_ms = msgE['time_ms']
            # print(f'Send message {j}. Yaw: {msg["groundCourse"]/10}')
            #print(msgE)
...
print("Starting Natnet3.x to Ivy/Udp interface at %s" % (args.server))
try:
