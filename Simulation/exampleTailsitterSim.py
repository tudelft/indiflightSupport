# PyNDIflight simulation definition for a tailsitter.
#
...
from PyNDIflight.crafts import MultiRotor, Rotor, IMU, TailSitterHover, BlownFlap
...
if __name__=="__main__":
    ...
    imu = IMU(mc, r=[-0.01, -0.012, 0.008], qBody=[0., -np.sqrt(2)/2., 0., -np.sqrt(2)/2], accStd=0.8, gyroStd=0.08)
    ...
    sil = IndiflightSITLWrapper(mc, imu, args.sil, N=4) if args.sil else None
    ...
    for i in tqdm(range(int(T / dt)), target_looptime=2*dt_rt):
        ...
        #if not atRef and sim.t > 17. and sil is not None:
